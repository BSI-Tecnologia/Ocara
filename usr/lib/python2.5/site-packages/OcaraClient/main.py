...
from OcaraCore.net.client import NetClient
from OcaraCore.ConfigClient import get_default_client

from OcaraClient.ui import icons
from OcaraClient.globals import *
from OcaraCore.utils import md5_cripto, kill_process
from OcaraCore.utils import get_os, humanize_time
from OcaraClient.ui import dialogs, login
from OcaraClient.ui.utils import get_gtk_builder
from OcaraClient.logout_actions import ActionManager
from OcaraClient.utils import HttpDownload

# Check DBus
try:
    from OcaraClient.dbus_manager import DbusManager
except ImportError:
...
class Client:
    ...
    cleanup_apps_id = 0
    cleanup_apps_timeout = 1
    currency = "R$" #by default
    interative = True
    login_attempts = 0
    blocked = True
    inf_login = True
	...
	self.login_user = ""
        ...
        self.user_logado = self.conf_client.get_string('user_logado')
	...
	self.main_window.set_size_request(300, 210)
        ...
        self.show_time_elapsed(False)
        self.show_time_remaining(True)

        self.xml.connect_signals(self)

        #Tray
        self.tray_icon = gtk.status_icon_new_from_file("/usr/share/OcaraClient/icons/ocara-icone.png")
        self.tray_icon.set_tooltip(_("Ocara - Cliente"))
    ...
    def set_information(self, data):
        ...
        if 'default_welcome_msg' in data:
            if data['default_welcome_msg']:
                self.login_window.set_title(_('Bem-vindo!'))
        ...
        if 'inf_login' in data:
            if data['inf_login']:
                self.login_window.lbl_info.visible(data['inf_login'])
            else:
                self.login_window.lbl_info.visible(True)
	...
	os.system("rm -f %s" %self.nome_arquivo)
	#os.system("rm -f /home/"+self.user_logado+"/paramiko.log")
        ...
        if action == 0 : #login ocara desktop
	...
	if 'full_name' in data:
            #self.full_name.set_use_markup(True)
	...
	self.cr = 1
        #deixar so isto para que comece a contar tempo assim que o usuario loga
	...
	if self.login_user == "" and self.full_name.get_text() == "":
	   self.nome_arquivo = '.'+self.user_logado+'-processos.txt'
	else:
	   self.nome_arquivo = '.'+self.full_name.get_text()+'-processos.txt'

    def editar_arquivo(self):

	#enviar informacoes dos processos que o usuario está executando para o servidor
	#cria arquivo
	arquivo = open(self.nome_arquivo,'w')
	proc = os.popen("ps --User " + self.user_logado + " -o pid,comm --sort -rss 2>/dev/null | grep -v PID | grep -v COMM | grep -v grep | grep -v startkde | grep -v gnome-session | grep -v x-session-manag")
        lista_process = proc.readlines()
        for process in lista_process:
            arquivo.write(process+'\n')
	...
	#envia arquivo
	import paramiko
	#paramiko.util.log_to_file("/home/"+self.user_logado+"/paramiko.log")
	host = "192.168.17.254"
	port = 22
	...
	localpath = self.nome_arquivo
	remotepath = "/home/tux/.processos/"+self.nome_arquivo
	...
	print 'TO AKI'
	    ...
	    self.tray_icon.set_tooltip(_("Faltam: %s para encerrar a sessão" % time_left_str))
	...
	if time_left_str == "00:02:00":
	   os.system("/usr/lib/python2.5/site-packages/OcaraClient/script &")
    ...
    def set_status(self, data):
        ...
        if 'time' in data:
            ...
            time_str = "%0.2d:%0.2d:00" % tuple(self.time)
    ...
    def on_login_response(self, obj, value):
        ...
        if value==True:
            ...
            self.login_window.err_box.set_text(
                            _("Usuário ou senha incorreta."))
        elif value==2:
            self.login_attempts = 0
            self.login_window.err_box.set_text(
                            _("Peça para o administrador tempo para utilizar a máquina."))

        elif value==3:
            self.login_attempts = 0
            self.login_window.err_box.set_text(
                            _("Permissao negada! Contacte seu administrador."))

	elif value==4:
	    self.login_attempts = 0
            self.login_window.err_box.set_text(
                            _("Usuário já está logado no sistema!"))
    ...
    def on_logout_menuitem_activate(self, obj):
        ...
        dlg.set_markup(_("<big><b>Voce realmente deseja sair do sistema agora?</b></big>\n\n"
                         "Se sair, tudo que nao for salvo sera perdido!"))
        response = dlg.run()
        dlg.destroy()

        if response == gtk.RESPONSE_YES:
            self.netclient.request('logout')
            #mandar logout para o sistema

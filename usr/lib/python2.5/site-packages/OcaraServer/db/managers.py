...
class OpenDebtsOtherManager(BaseFlow):
    ...
    def get_debts_by_date(self, year, month_ini, day_ini):
        s = select([OpenDebtOtherItem.day,OpenDebtOtherItem.value],
              and_(OpenDebtOtherItem.year==year,
                   OpenDebtOtherItem.month==month_ini,
                   OpenDebtOtherItem.day==day_ini))
        days = []

        for day, value in self.db_session.session.execute(s):
	    print value
            if not day in days:

...
class HistoryManager(BaseFlow):
    ...
    def query_relatorio_utilizacao_usuario(self, from_date, to_date, user_id):
        s = select([HistoryItem.day, HistoryItem.month, HistoryItem.year, HistoryItem.start_time, HistoryItem.end_time, HistoryItem.time], and_(HistoryItem.user_id == user_id, HistoryItem.c.year.between(from_date[0], to_date[0]), HistoryItem.c.month.between(from_date[1], to_date[1]), HistoryItem.c.day.between(from_date[2], to_date[2])))
        linhas_relatorio = []
        for row in self.db_session.session.execute(s):
            linhas_relatorio.append([row['day'], row['month'], row['year'], row['start_time'], row['end_time'], row['time']])

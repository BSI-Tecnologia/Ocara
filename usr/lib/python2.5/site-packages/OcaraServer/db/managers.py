...
from sqlalchemy.sql import select, and_, or_, delete, update, func, exists
...
class ScheduleOfBorrowManager(BaseManager):
    ...
    def get_by_period_and_heritage_id(self, heritage_id, from_date, to_date):
        s = select([ScheduleOfBorrow.borrowed_date, User.full_name, ScheduleOfBorrow.return_date], and_(Heritage.c.id == heritage_id, ScheduleOfBorrow.c.id == Heritage.c.borrow_id, ScheduleOfBorrow.c.borrowed_date.between(from_date, to_date) ) )
        return self.db_session.session.execute(s).fetchall()
...
class CashFlowManager(BaseFlow):
    ...
    def get_debts_by_date(self, year_ini, month_ini, day_ini, year_fim, month_fim, day_fim):
        s = select([CashFlowItem.value],
              and_(CashFlowItem.year.between(year_ini, year_fim),
                   CashFlowItem.month.between(month_ini, month_fim),
                   CashFlowItem.day.between(day_ini, day_fim)))
...
class OpenDebtsMachineManager(BaseFlow):
    ...
    def get_debts_by_date(self, year_ini, month_ini, day_ini, year_fim, month_fim, day_fim):
        s = select([OpenDebtMachineItem.value],
              and_(OpenDebtMachineItem.year.between(year_ini, year_fim),
                   OpenDebtMachineItem.month.between(month_ini, month_fim),
                   OpenDebtMachineItem.day.between(day_ini, day_fim)))
    ...
    def get_debts_by_user(self, user_id):
        s = select([OpenDebtMachineItem.day, OpenDebtMachineItem.month, OpenDebtMachineItem.year, OpenDebtMachineItem.start_time, OpenDebtMachineItem.end_time, OpenDebtMachineItem.value],
...
class OpenDebtsOtherManager(BaseFlow):
    ...
    def get_debts_by_date(self, year_ini, month_ini, day_ini, year_fim, month_fim, day_fim):
        s = select([OpenDebtOtherItem.value],
              and_(OpenDebtOtherItem.year.between(year_ini, year_fim),
                   OpenDebtOtherItem.month.between(month_ini, month_fim),
                   OpenDebtOtherItem.day.between(day_ini, day_fim)))
	...
	return valores
    def get_debts_by_user(self, user_id):
        s = select([OpenDebtOtherItem.day, OpenDebtOtherItem.month, OpenDebtOtherItem.year, OpenDebtOtherItem.start_time, OpenDebtOtherItem.end_time, OpenDebtOtherItem.value],
...
class HistoryManager(BaseFlow):
    ...
    def query_relatorio_utilizacao_usuario(self, from_date, to_date, user_id):
        s = select([HistoryItem.day, HistoryItem.month, HistoryItem.year, HistoryItem.start_time, HistoryItem.end_time, HistoryItem.time, HistoryItem.machine_id], and_(HistoryItem.user_id == user_id))
        linhas_relatorio = []
    ...
    def query_relatorio_utilizacao_machine(self, machine_mac, from_date, to_date):
        s = select([HistoryItem.day, HistoryItem.month, HistoryItem.year, HistoryItem.start_time, HistoryItem.end_time, HistoryItem.time, HistoryItem.machine_id], and_(HistoryItem.machine_id == machine_mac, HistoryItem.c.year.between(from_date[0], to_date[0]), HistoryItem.c.month.between(from_date[1], to_date[1]), HistoryItem.c.day.between(from_date[2], to_date[2])))
        linhas_relatorio = []
        for row in self.db_session.session.execute(s):
            linhas_relatorio.append([row['day'], row['month'], row['year'], row['start_time'], row['end_time'], row['time'], row['machine_id']])
        return linhas_relatorio

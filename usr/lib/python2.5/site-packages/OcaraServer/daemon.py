...
from OcaraServer.db.managers import MachineManager, UserManager, CashFlowManager, PerfilManager, PerfilUserManager
from OcaraServer.db.managers import OpenDebtsMachineManager, OpenDebtsOtherManager, OpenDebtsMachineBckManager
from OcaraServer.db.managers import ActionManager, ActionCategoryManager, PerfilActionManager
...
class MachineInst(gobject.GObject):
        ...
        if self.session:
            ...
            if registred:
                ...
                if ((credit == 0) or credit < self.creditN):
                    ...
                    oitem.year = today.year
                    oitem.month = today.month
                    oitem.day = today.day
                    oitem.start_time = datetime.datetime.now().strftime(_("%H:%M:%S"))
                    oitem.end_time = datetime.datetime.now().strftime(_("%H:%M:%S"))
                    oitem.user_id = user_id
                    oitem.machine_id = m.id
                    oitem.value = credit
                    oitem.notes = "Quitação de dívida em aberto pela utilização de máquina."
    ...
    def quit_application(self, machine_inst):
        """
...
class Server(gobject.GObject):
    ...
    def __init__(self):
        ...
        ###BSI
        self.perfil_manager = PerfilManager(self.db_session)
        self.perfil_user_manager = PerfilUserManager(self.db_session)
        self.action_manager = ActionManager(self.db_session)
        self.action_category_manager = ActionCategoryManager(self.db_session)
        self.perfil_action_manager = PerfilActionManager(self.db_session)
        self.opendebts_machine_bck_manager = OpenDebtsMachineBckManager(self.db_session)
        ###BSI

...
#	self.on_cash_flow_report_generated_clicked(self)
...
class fatura_frequentador:
    ...
    def usuariosComboBoxValueChange(self, combobox):
        index = combobox.get_active()
        self.nick = self.usuario_list[index].nick
        return
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if self.nick=="":
            ok_only(_("<b>É necesssário informar um usuário válido.</b>\n\n"), Parent=None, ICON=gtk.MESSAGE_ERROR)
            return
        if selected_user:
            days = []
            for line in self.manager.open_debts_other_manager.get_debts_by_user_date(selected_user.id, date_ini, date_fim): #Itens de debitos outros
                ...
                tempo_ini = line[3].split(':',2)
                tempo_fim = line[4].split(':',2)
                fim = datetime.datetime(2000, 01, 01, int(tempo_fim[0]), int(tempo_fim[1]), int(tempo_fim[2]))
                inicio = datetime.datetime(2000, 01, 01, int(tempo_ini[0]), int(tempo_ini[1]), int(tempo_ini[2]))
                tempo_utilizado = fim - inicio
                self.btn_print.set_sensitive(True)
                restCashValor += float(line[5]) - cashflowValor
                self.opendebtsStore.append(None, [ date_time, 'Outros', "R$: %.2f" % line[5], "R$: %.2f" % cashflowValor, "R$: %.2f" % restCashValor])
            ...
            self.total.show_all()
            self.header = 'Relatório de Débitos em Aberto\n\r Usuário: ' + self.nick + '     Periodo: ' + str("%s-%s-%s" % (date_ini[0], date_ini[1], date_ini[2])) + ' à ' + str("%s-%s-%s" % (date_fim[0], date_fim[1], date_fim[2])) + '\n\r'
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if selected_user:
            for line in self.manager.open_debts_other_manager.get_debts_by_user_date(selected_user.id, date_ini, date_fim): #Itens de debitos outros
                date_time = datetime.date(int(line[0]), int(line[1]), int(line[2]))
                cashflowValor = 0
                restCashValor += int(line[5]) - int(cashflowValor)

...
class relatorio_utilizacao_frequentador:
    ...
    def on_pesquisar_clicked(self, obj):
        ...
        for item in result_open_debts:
            liststore_open_debts.append([ ("%s-%s-%s" % (item[0], item[1], item[2])), item[3], "R$: %.2f" % item[5]])
            total_valor_debts += float(item[6])
        ...
        for item in result:
            ...
            time_delta_inicio = datetime.timedelta(hours=int_list_inicio[0], minutes=int_list_inicio[1], seconds=int_list_inicio[2])

            int_list_fim = [int(i) for i in item[4].split(":")]
            time_delta_fim = datetime.timedelta(hours=int_list_fim[0], minutes=int_list_fim[1], seconds=int_list_fim[2])
            ...
            total_tempo += datetime.timedelta(hours=int(hora), minutes=int(minuto), seconds=int(segundo))
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if selected_user:
            ...
            for line in self.manager.open_debts_other_manager.get_debts_by_user_date(selected_user.id, date_ini, date_fim): #Itens de debitos outros
                ...
                self.opendebtsStore.append(None, [ ("%s-%s-%s" % (line[2], line[1], line[0])), 'Outros', "R$: %.2f" % line[5], "R$: %.2f" % cashflowValor, "R$: %.2f" % restCashValor])
                self.data = self.data + str(date_time) + '     Outros         ' + str("R$: %.2f" % line[5]) + ' -    ' + str("R$: %.2f" % cashflowValor) + ' -   ' + str("R$: %.2f" % restCashValor) + '\n'
            ...
            for line in self.manager.open_debts_machine_manager.get_debts_by_user_date(selected_user.id, date_ini, date_fim): #Itens de debitos maquinas
                ...
                self.btn_print.set_sensitive(True)
                restCashValor += float(line[5]) - cashflowValor
                self.opendebtsStore.append(None, [ ("%s-%s-%s" % (line[2], line[1], line[0])), 'Internet', "R$: %.2f" % line[5], "R$: %.2f" % cashflowValor, "R$: %.2f" % restCashValor])
                self.data = self.data + str("%s-%s-%s" % (line[2], line[1], line[0])) + '     Internet         ' + str("R$: %.2f" % line[5]) + ' -    ' + str("R$: %.2f" % cashflowValor) + ' -   ' + str("R$: %.2f" % restCashValor) + '\n'
                strtotal = "Total: " + str("R$: %.2f" % restCashValor)
...
class relatorio_utilizacao_maquina:
    ...
    def on_pesquisar_clicked(self, obj):
        ...
        for line in result: #Itens de debitos outros
            ...
            delta = datetime.timedelta()
            total_maq +=  datetime.timedelta(hours=hora, minutes=minuto, seconds=segundo)
            delta = datetime.timedelta(hours=hora, minutes=minuto, seconds=segundo) / line[4]
            strdelta = str(delta).split(".")
            liststore.append([ ("%s-%s-%s" % (line[0], line[1], line[2])), str(line[3]), str(line[4]), strdelta[0]])

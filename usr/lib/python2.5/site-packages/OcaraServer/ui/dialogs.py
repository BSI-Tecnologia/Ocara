...
class relatorio_utilizacao_maquina:
    ...
    def on_pesquisar_clicked(self, obj):
        ...
        for line in result: #Itens de debitos outros
            ...
            string_data = str(line[0]) + '/' + str(line[1]) + '/' + str(line[2])

            if not date_time in days: #Testa se já usou o pagamentos do dia
                days.append(date_time)
                price_per_hour = self.manager.machine_category_manager.get_price_by_machine_id(line[6])
                if not price_per_hour:
                    price_per_hour = get_default_client().get_float('price_per_hour')
            else:
                total_tempo = 0
            ...
            #restCashValor += cashflowValor
            #liststore.append(None, [ ("%s-%s-%s" % (line[2], line[1], line[0])), str(total_tempo), str(users), 'Tempo', ("R$: %.2f" % 0 )])
            liststore.append(None, [ ("%s-%s-%s" % (line[2], line[1], line[0])), '123', "123", 'Tempo', '132'])
            #strtotal = "Total: " + str("R$: %.2f" % restCashValor)

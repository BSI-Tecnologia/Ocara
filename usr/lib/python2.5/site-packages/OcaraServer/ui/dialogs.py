...
#	self.on_cash_flow_report_generated_clicked(self)
	...
	if datetime.date(int(self.year_ini), int(self.month_ini), int(self.day_ini)) > datetime.date(int(self.year_fim), int(self.month_fim), int(self.day_fim)):
  	...
  	self.cashflowStore.append(None, ['R$ ' + str(cashflowValor), 'R$ ' + str(opendebtsotherValor)])
    def on_cancel_button_clicked(self, obj):
...
class fatura_frequentador:
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if out:
            days = []
            for line in self.manager.open_debts_other_manager.get_debts_by_user_date(out[1], date_ini, date_fim): #Itens de debitos outros
            ...
            for line in self.manager.open_debts_machine_manager.get_debts_by_user_date(out[1], date_ini, date_fim): #Itens de debitos maquinas
                date_time = datetime.date(int(line[0]), int(line[1]), int(line[2]))
                cashflowValor = 0
            ...
            label = gtk.Label()
            label.set_markup("<b>Total: " + str("R$: %.2f" % restCashValor) + '</b>')
            label.set_use_underline(True)
            label.set_alignment(0, 0.50)
            self.total.attach(child=label,
                     left_attach=0,
                     right_attach=1,
                     top_attach=0,
                     bottom_attach=1,
...
class baixa_debts_utilizador:
    ...
    def on_toggled(self, widget, path):
        current_value = self.opendebtsStore[path][5]
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if out:
            ...
            for line in self.manager.open_debts_machine_manager.get_debts_by_user_date(out[1], date_ini, date_fim):
                date_time = datetime.date(int(line[0]), int(line[1]), int(line[2]))
                cashflowValor = 0
                restCashValor += int(line[5]) - int(cashflowValor)

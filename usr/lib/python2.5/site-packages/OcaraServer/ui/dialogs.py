...
#	self.on_cash_flow_report_generated_clicked(self)
	...
	if datetime.date(int(self.year_ini), int(self.month_ini), int(self.day_ini)) > datetime.date(int(self.year_fim), int(self.month_fim), int(self.day_fim)):
	    ...
	    opendebtsotherValor += line['value']
  	...
  	self.cashflowStore.append(None, [cashflowValor, opendebtsotherValor])
    def on_cancel_button_clicked(self, obj):
...
class fatura_frequentador:
    ...
    def __init__(self, manager):
        ...
        self.table = self.xml.get_object('table1')
        self.cols = ['Data','Serviço','Tempo de utilização','Valor Total','Valor Pago','Valor a ser Pago']
        sequence = [str] * len(self.cols)
...
#	self.lbl_dt_ini.set_markup("00/00/00")
#	self.lbl_dt_fim.set_markup("00/00/00")
	restCashValor = 0
	self.btn_print.set_sensitive(False)
	self.opendebtsStore.clear()
        ...
        if out:
            days = []
            for line in self.manager.open_debts_other_manager.get_debts_by_user(out[1], date_ini, date_fim): #Itens de debitos outros
	    	date_time = datetime.date(int(line['year']), int(line['month']), int(line['day']))
		string_data = str(line['day']) + '/' + str(line['month']) + '/' + str(line['year'])
	    	cashflowValor = 0
	    	if not date_time in days: #Testa se já usou o pagamentos do dia
		    if date_time < date_ini:
		    	date_ini = date_time
		    elif date_time > date_fim:
		    	date_fim = date_time
		    days.append(date_time)
            	    for cash in self.manager.cash_flow_manager.get_debts_by_date(int(line['year']), int(line['month']), int(line['day']), int(line['year']), int(line['month']), int(line['day'])):
	            	cashflowValor += (cash['value'] + restCashValor)
	    	elif cashflowValor < 0: #Testa se há saldo a devolver
	    	    cashflowValor = restCashValor * -1

		tempo_ini = line['start_time'].split(':',2)
		tempo_fim = line['end_time'].split(':',2)
		...
		#self.lbl_dt_ini.set_markup(str(date_ini))
		#self.lbl_dt_fim.set_markup(str(date_fim))
	    	restCashValor += int(line['value']) - int(cashflowValor)
	    	self.opendebtsStore.append(None, [ date_time, 'Outros', tempo_utilizado, line['value'], cashflowValor, restCashValor])
		self.data = self.data + str(date_time) + '     Outros     ' + str(tempo_utilizado) + '    R$:' + str(line['value']) + ' -    R$:' + str(cashflowValor) + ' -   R$:' + str(restCashValor) + '\n\r'

            for line in self.manager.open_debts_machine_manager.get_debts_by_user(out[1], date_ini, date_fim): #Itens de debitos maquinas
	    	date_time = datetime.date(int(line['year']), int(line['month']), int(line['day']))
	    	cashflowValor = 0
		    ...
		    if date_time < date_ini:
		    	...
		    	date_fim = date_time
		    days.append(date_time)
            	    for cash in self.manager.cash_flow_manager.get_debts_by_date(int(line['year']), int(line['month']), int(line['day']), int(line['year']), int(line['month']), int(line['day'])): #recupera o pagamento do dia
	            	cashflowValor += (cash['value'] + restCashValor)
	    	elif cashflowValor < 0: #Testa se há saldo a devolver
	    	    cashflowValor = restCashValor * -1
		...
		#self.lbl_dt_fim.set_markup(str(date_fim))
	    	restCashValor += int(line['value']) - int(cashflowValor)
	    	self.opendebtsStore.append(None, [ date_time, 'Internet', tempo_utilizado, line['value'], cashflowValor, restCashValor])
		self.data = self.data + str(date_time) + '     Internet     ' + str(tempo_utilizado) + '    R$:' + str(line['value']) + ' -    R$:' + str(cashflowValor) + ' -   R$:' + str(restCashValor) + '\n\r'
...
class baixa_debts_utilizador:
    ...
    def __init__(self, manager):
        ...
        self.cols = ['id','Data','Serviço','Hora Inicial','Hora Final','Valor do Serviço']
        #sequence = [str] * (len(self.cols)+1)#coluna adicional com checkbox
        self.opendebtsStore = gtk.TreeStore(str, str, str, str, str, str, bool)
        self.opendebtsView = gtk.TreeView(self.opendebtsStore)
    ...
    def run(self):
        self.xml.get_object("hbox_ini").pack_start(self.data_inicio)
        self.data_inicio.show()
        self.daily_calendar = self.data_inicio.get_internal_calendar()

        self.xml.get_object("hbox_fim").pack_start(self.data_fim)
        self.data_fim.show()
        self.daily_calendar_fim = self.data_fim.get_internal_calendar()
        ...
        renderer_in_out = gtk.CellRendererToggle()
        renderer_in_out.connect("toggled", self.on_toggled)
        column_in_out = gtk.TreeViewColumn("Débitos a Quitar", renderer_in_out, active=6)
    ...
    def on_toggled(self, widget, path):
        current_value = self.opendebtsStore[path][6]
        debt_id = int(self.opendebtsStore[path][0])

        if current_value:
            self.opendebtsStore[path][6] = False
            self.debts.remove(debt_id)
        else:
            self.opendebtsStore[path][6] = True
            self.debts.append(debt_id)
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        date_ini = self.data_inicio.get_time()
        date_fim = self.data_fim.get_time()
        restCashValor = 0
        self.btn_print.set_sensitive(False)
        self.opendebtsStore.clear()
        ...
        if nick == "":
            ...
            return
        if out:
            for line in self.manager.open_debts_machine_manager.get_debts_by_user(out[1], date_ini, date_fim):
                date_time = datetime.date(int(line['year']), int(line['month']), int(line['day']))
                cashflowValor = 0
                restCashValor += int(line['value']) - int(cashflowValor)
                self.opendebtsStore.append(None, [ line['id'], date_time, 'Internet', line['start_time'], line['end_time'], line['value'], bool(False)])
                self.data = self.data + line['start_time'] + '  ' +str(date_time) + '  Internet  ' + line['start_time'] + ' - ' + line['end_time'] + '    R$:' + str(line['value']) + ' -  R$:' + str(restCashValor) + '\n\n'
            self.header = 'Usuário: ' + nick + '\n\r Periodo: ' + str(date_ini) + ' à ' + str(date_fim) + '\n\r'
            self.header += self.data
        else:
            ok_only(_("<b>Usuário não é válido.</b>\n\n"), Parent=None, ICON=gtk.MESSAGE_ERROR)

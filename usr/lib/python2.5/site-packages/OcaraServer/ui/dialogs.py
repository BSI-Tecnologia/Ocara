...
class baixa_debts_utilizador:
    ...
    def run(self):
        ...
        renderer_in_out = gtk.CellRendererToggle()
        column_in_out = gtk.TreeViewColumn("Débitos a Quitar", renderer_in_out, active=1)
    ...
    def on_toggled(self, widget, path):
        current_value = self.store[path][1]
        self.opendebtsStore[path][1] = not current_value
        current_value = not current_value
        if len(path) == 1:
            piter = self.opendebtsStore.get_iter(path)
            citer = self.opendebtsStore.iter_children(piter)
            while citer is not None:
                self.opendebtsStore[citer][1] = current_value
                citer = self.opendebtsStore.iter_next(citer)
        elif len(path) != 1:
            citer = self.opendebtsStore.get_iter(path)
            piter = self.opendebtsStore.iter_parent(citer)
            citer = self.opendebtsStore.iter_children(piter)
            all_selected = True
            while citer is not None:
                if self.opendebtsStore[citer][1] is False:
                    all_selected = False
                    break
                citer = self.opendebtsStore.iter_next(citer)
            self.opendebtsStore[piter][1] = all_selected
	...
	if out:
		...
		string_data = str(line['day']) + '/' + str(line['month']) + '/' + str(line['year'])
	    	...
	    	restCashValor += int(line['value']) - int(cashflowValor)
	    	self.opendebtsStore.append(None, [ date_time, 'Internet', line['start_time'], line['end_time'], line['value'], False])
		self.data = self.data + str(date_time) + '  Internet  ' + line['start_time'] + ' - ' + line['end_time'] + '    R$:' + str(line['value']) + ' -  R$:' + str(restCashValor) + '\n\n'

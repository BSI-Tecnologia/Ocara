...
class relatorio_utilizacao_bens:
    ...
    def on_pesquisar_clicked(self, obj):
        ...
        for row in result:
            delta = row[2] - row[0]
            duracao_emprestimo = delta.days if delta.days >= 0 else 0
...
class relatorio_utilizacao_maquina:
    ...
    def __init__(self, manager):
        ...
        self.bens_list = self.manager.history_manager.get_all()
	...
	data = []
        for list_index, bem in enumerate(self.bens_list):
	    if not bem.machine_id in data:
		data.append(bem.machine_id)
            	self.bensListStore.append((int(list_index), "Máquina " + str(bem.machine_id)))
    ...
    def on_pesquisar_clicked(self, obj):
        ...
        for line in result: #Itens de debitos outros
            ...
            liststore.append([ ("%s-%s-%s" % (line[2], line[1], line[0])), str(line[3]), str(line[4]), delta, ("R$: %.2f" % cashflowValor )])
            strtotal = "Total Utilização: %s" % total_maq


        column1 = gtk.TreeViewColumn("Data Utilização")
        treeview.append_column(column1)
        column2 = gtk.TreeViewColumn("Tempo")
        treeview.append_column(column2)
    ...
    def bensComboBoxValueChange(self, combobox):
        #index = combobox.get_active()
        #self.xml.get_object('label_machine').set_text(str(self.bens_list[index].machine_id))
        return

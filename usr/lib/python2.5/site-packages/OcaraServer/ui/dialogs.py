...
from OcaraServer.ui import icons, DateEdit, HourEntry, PrintBase
...
class user_edit:
	...
	self.autorizoEnvio.set_active(user.autorizoEnvio)
        ...
        self.perfilComboBox.set_sensitive(False)
		...
		schedule.__setattr__(key, value)
            ...
            try:
                ...
                if reserve.id_user == schedule.id_user:
                    ...
                    schedule.heritage_id = self.heritage.id
...
class authentication_dialog:
    ...
    def __init__(self, manager, show_dialog):
        self.manager = manager
    ...
    def run(self):
        ...
        self.lbl_gerente_info = self.xml.get_object('lbl_info')
        inf_gerente_login = get_default_client().get_bool('inf_login')

        #self.lbl_gerente_info.visible(inf_gerente_login)
        if not inf_gerente_login:
            self.lbl_gerente_info.set_text("Para login utilize usuário: gerente / senha: 123 \n e desative essa informação no menu Editar > Preferências.")

        self.xml.connect_signals(self)
        self.autenticacao.show_all()

        if self.show_dialog:
            certeza = exit(_('<big><b>Você tem certeza que deseja trocar de usuário?'
                             '</b></big>'),
                           Parent=self.autenticacao, allow_miminize=False)
            if certeza.response == gtk.RESPONSE_CANCEL:
                self.autenticacao.destroy()
                return True
        self.manager.xml.get_object("usuario_logado_label").set_label("Usuário logado: ")
    ...
    def on_apply_bnt_clicked(self, obj):
        ...
        self.manager.xml.get_object("usuario_logado_label").set_label("Usuário logado: %s" % user.nick)
        self.autenticacao.destroy()
...
class relatorio_utilizacao_frequentador:

    def __init__(self, manager):
        self.manager = manager
        self.xml = get_gtk_builder('relatorio_utilizacao_frequentador')
        self.usuario_list = self.manager.users_manager.get_all()
        self.relatorio = self.xml.get_object('relatorio_utilizacao_frequentador')
        self.table = self.xml.get_object('table1')
        self.cancel_bnt = self.xml.get_object('cancel_bnt')
        self.xml.connect_signals(self)
        self.data_inicio = DateEdit.DateEdit()
        self.data_fim = DateEdit.DateEdit()
        self.usuariosListStore = gtk.ListStore(gobject.TYPE_INT, gobject.TYPE_STRING)
        self.usuariosComboBox = gtk.ComboBox(model=self.usuariosListStore)
        self.usuariosComboBox.connect('changed', self.usuariosComboBoxValueChange)

    def run(self):
        self.table.attach(self.data_inicio, 1, 2, 2, 3)
        self.table.attach(self.data_fim, 2, 3, 2, 3)

        cell = gtk.CellRendererText()
        self.usuariosComboBox.pack_start(cell, True)
        self.usuariosComboBox.add_attribute(cell, 'text', 1)
        self.usuariosComboBox.set_row_separator_func(self.row_separator_func)

        self.table.attach(child=self.usuariosComboBox,
                     left_attach=1,
                     right_attach=2,
                     top_attach=0,
                     bottom_attach=1,
        ...
        for list_index, usuario in enumerate(self.usuario_list):
            self.usuariosListStore.append((int(list_index), usuario.nick))

        self.usuariosComboBox.show()

        self.relatorio.show_all()

    def on_pesquisar_clicked(self, obj):
        str_inicio = self.data_inicio.get_time()
        str_fim = self.data_fim.get_time()
        selected_index = self.usuariosComboBox.get_active()
        selected_user = self.usuario_list[selected_index]

        date_inicio = date(str_inicio[0], str_inicio[1], str_inicio[2])
        date_fim = date(str_fim[0], str_fim[1], str_fim[2])

        if date_inicio > date_fim:
            ok_only(_("<b>Período de pesquisa inválido. Data inical não pode ser maior que a data final.</b>\n\n"), Parent=None, ICON=gtk.MESSAGE_ERROR)
            return True

        result = self.manager.history_manager.query_relatorio_utilizacao_usuario(str_inicio, str_fim, selected_user.id)
        result_open_debts = self.manager.open_debts_machine_manager.get_debts_by_user_date(selected_user.id, str_inicio, str_fim)

        try:
            for index, item in enumerate(self.xml.get_object('vbox1').get_children()):
                if index > 1:
                    item.destroy()
        except Exception, error:
            print error

        scrolled_window_open_debts = gtk.ScrolledWindow()
        scrolled_window_open_debts.set_border_width(10)
        scrolled_window_open_debts.set_policy(gtk.POLICY_AUTOMATIC, gtk.POLICY_AUTOMATIC)

        liststore_open_debts = gtk.ListStore(str, str, str)
        treeview_open_debts = gtk.TreeView(liststore_open_debts)
        treeview_open_debts.set_grid_lines(gtk.TREE_VIEW_GRID_LINES_BOTH)

        total_valor_debts = 0.0

        for item in result_open_debts:
            liststore_open_debts.append([ ("%s-%s-%s" % (item[2], item[1], item[0])), item[3], "R$: %.2f" % item[5]])
            total_valor_debts += float(item[5])

        open_debts_column1 = gtk.TreeViewColumn("Data")
        treeview_open_debts.append_column(open_debts_column1)
        open_debts_column2 = gtk.TreeViewColumn("Hora Inicio")
        treeview_open_debts.append_column(open_debts_column2)
        open_debts_column3 = gtk.TreeViewColumn("Valor")
        treeview_open_debts.append_column(open_debts_column3)

        cell = gtk.CellRendererText()
        open_debts_column1.pack_start(cell, True)
        open_debts_column1.add_attribute(cell, "text", 0)
        open_debts_column2.pack_start(cell, True)
        open_debts_column2.add_attribute(cell, "text", 1)
        open_debts_column3.pack_start(cell, True)
        open_debts_column3.add_attribute(cell, "text", 2)
        ...
        total_valor = 0.0
        total_tempo = datetime.timedelta()

        for item in result:
            price_per_hour = self.manager.machine_category_manager.get_price_by_machine_id(item[6])
            if not price_per_hour:
                price_per_hour = get_default_client().get_float('price_per_hour')
            ### duracao
            int_list_inicio = [int(i) for i in item[3].split(":")]
            if int_list_inicio[2] > 30:
                    int_list_inicio[1] += 1
            time_delta_inicio = datetime.timedelta(hours=int_list_inicio[0], minutes=int_list_inicio[1], seconds=0)

            int_list_fim = [int(i) for i in item[4].split(":")]
            if int_list_fim[2] > 30:
                    int_list_fim[1] += 1
            time_delta_fim = datetime.timedelta(hours=int_list_fim[0], minutes=int_list_fim[1], seconds=0)

            delta = datetime.timedelta()
            delta = time_delta_fim - time_delta_inicio
            ### duracao

            time = str(delta).split(":")
            hora = float(time[0])
            minuto = float(time[1])
            segundo = float(time[2])
            if segundo > 30:
                    minuto += 1
            liststore.append([ ("%s-%s-%s" % (item[0], item[1], item[2])), item[3], item[4], str(delta), ("R$: %.2f" % calculate_credit(price_per_hour, hora, minuto) ) ])
            total_valor += calculate_credit(price_per_hour, hora, minuto)
            total_tempo += datetime.timedelta(hours=int(hora), minutes=int(minuto), seconds=int(0))



        column1 = gtk.TreeViewColumn("Data")
        treeview.append_column(column1)
        column2 = gtk.TreeViewColumn("Hora início")
        treeview.append_column(column2)
        column3 = gtk.TreeViewColumn("Hora fim")
        treeview.append_column(column3)
        column4 = gtk.TreeViewColumn("Duração")
        treeview.append_column(column4)
        column5 = gtk.TreeViewColumn("Valor")
        ...
        column5.pack_start(cell, True)
        column5.add_attribute(cell, "text", 4)

        title1 = gtk.Label()
        title1.set_markup("<b>Pagos</b>")

        title2 = gtk.Label()
        title2.set_markup("<b>Em aberto</b>")

        title3 = gtk.Label()
        title3.set_markup("<i>Tempo total = %s Valor total pago= R$: %.2f</i>" % (str(total_tempo),total_valor,))

        title4 = gtk.Label()
        title4.set_markup("<i>Valor total aberto = R$: %.2f</i>" % total_valor_debts)

        scrolled_window.add_with_viewport(treeview)
        scrolled_window.set_size_request(400, 200)

        scrolled_window_open_debts.add_with_viewport(treeview_open_debts)
        scrolled_window_open_debts.set_size_request(400, 200)

        self.xml.get_object('vbox1').add(title1)
        self.xml.get_object('vbox1').add(scrolled_window)
        self.xml.get_object('vbox1').add(title3)
        self.xml.get_object('vbox1').add(title2)
        self.xml.get_object('vbox1').add(scrolled_window_open_debts)
        self.xml.get_object('vbox1').add(title4)
        self.relatorio.show_all()
    ...
    def usuariosComboBoxValueChange(self, combobox):
        index = combobox.get_active()
        cpf = self.usuario_list[index].cpf
        if not cpf:
            cpf = "Não informado."
        self.xml.get_object('label_nro_bem').set_text(cpf)
        return


class relatorio_utilizacao_bens:

    def __init__(self, manager):
        self.manager = manager
        self.xml = get_gtk_builder('relatorio_utilizacao_bens')
        self.bens_list = self.manager.list_bens_manager.get_all()
        self.relatorio = self.xml.get_object('relatorio_utilizacao_bens')
        self.table = self.xml.get_object('table1')
        self.cancel_bnt = self.xml.get_object('cancel_bnt')
        self.xml.connect_signals(self)
        self.data_inicio = DateEdit.DateEdit()
        self.data_fim = DateEdit.DateEdit()
        self.bensListStore = gtk.ListStore(gobject.TYPE_INT, gobject.TYPE_STRING)
        self.bensComboBox = gtk.ComboBox(model=self.bensListStore)
        self.bensComboBox.connect('changed', self.bensComboBoxValueChange)
    ...
    def run(self):
        ...
        cell = gtk.CellRendererText()
        self.bensComboBox.pack_start(cell, True)
        self.bensComboBox.add_attribute(cell, 'text', 1)
        self.bensComboBox.set_row_separator_func(self.row_separator_func)

        self.table.attach(child=self.bensComboBox,
                     left_attach=1,
                     right_attach=2,
                     top_attach=0,
                     bottom_attach=1,
        ...
        for list_index, bem in enumerate(self.bens_list):
            self.bensListStore.append((int(list_index), bem.name))

        self.bensComboBox.show()
    ...
    def on_pesquisar_clicked(self, obj):
        ...
        data_inicio_selected = datetime.date(int(str_inicio[0]), int(str_inicio[1]), int(str_inicio[2]) )
        data_fim_selected = datetime.date(int(str_fim[0]), int(str_fim[1]), int(str_fim[2]))
        data_fim_selected = datetime.timedelta(days=1) + data_fim_selected
        selected_index = self.bensComboBox.get_active()
        selected_bem = self.bens_list[selected_index]
        result = self.manager.schedule_manager.get_by_period_and_heritage_id(selected_bem.id, data_inicio_selected, data_fim_selected)
        ...
        for row in result:
            #delta = row[2] - row[0]
            delta = datetime.date((row[2]).year, (row[2]).month, (row[2]).day) - datetime.date((row[0]).year, (row[0]).month, (row[0]).day)
            duracao_emprestimo = delta.days if delta.days > 0 else 1
            total_days += duracao_emprestimo

            liststore.append([row[0].strftime('%d/%m/%Y'), row[1], row[2].strftime('%d/%m/%Y'), duracao_emprestimo])

        column1 = gtk.TreeViewColumn("Data Empréstimo")
        treeview.append_column(column1)
        column2 = gtk.TreeViewColumn("Nome Usuário")
        treeview.append_column(column2)
        column3 = gtk.TreeViewColumn("Data Devolução")
        treeview.append_column(column3)
        column4 = gtk.TreeViewColumn("Duração")
        treeview.append_column(column4)
        ...
        total = gtk.Label()
        total.set_markup("<b>Total de dias: %s</b>" % total_days)

        self.xml.get_object('vbox1').add(total);
    ...
    def bensComboBoxValueChange(self, combobox):
        index = combobox.get_active()
        self.xml.get_object('label_nro_bem').set_text(self.bens_list[index].number_ben)
        return
...
class lista_perfil:
    ...
    def on_delete_perfil_clicked(self, obj):
        ...
        tem_relacionamento_msg = None
        if tem_certeza.response:
            try:
                self.manager.perfil_manager.delete_by_id(self.perfil_selecionado.id)
            except IntegrityError:
                self.manager.perfil_manager.rollback()
                tem_relacionamento_msg = tem_certeza = yes_no("Este perfil esta associado a outras entidades do sistema, para remove-lo é necessario remover esse(s) relacionamento(s). Confirma?")
            finally:
                self.manager.perfil_manager.session_close()

        if tem_relacionamento_msg:
            if tem_relacionamento_msg.response:
                self.manager.perfil_action_manager.delete_by_perfil_id(self.perfil_selecionado.id)
                self.manager.perfil_user_manager.delete_by_perfil_id(self.perfil_selecionado.id)
                self.manager.perfil_manager.delete_by_id(self.perfil_selecionado.id)
...
class consulta_movimento:
    ...
    def __init__(self, manager):
        ...
        self.cols = ['Crédito Realizado','Crédito não realizado']
        sequence = [str] * len(self.cols)
        self.cashflowStore = gtk.TreeStore(* sequence)
        self.cashflowView = gtk.TreeView(self.cashflowStore)
        self.year_ini, self.month_ini, self.day_ini = localtime()[0:3]
        self.year_fim, self.month_fim, self.day_fim = localtime()[0:3]
    ...
    def run(self):
        ...
        self.cancel_btn = self.xml.get_object('cancel_btn')
        self.xml.connect_signals(self)


        self.cancel_btn = gtk.Button(stock=gtk.STOCK_CANCEL)
        self.cancel_btn.connect("clicked", self.on_cancel_button_clicked)
        ...
        self.daily_calendar.connect("day-selected",
                                    self.on_cash_flow_calendar_ini_selected)

        self.daily_calendar_fim.connect("day-selected",
                                    self.on_cash_flow_calendar_fim_selected)
    ...
    def on_cash_flow_calendar_ini_selected(self, obj):
        self.year_ini, self.month_ini, self.day_ini = obj.get_date()
        self.month_ini += 1 #Change range(0, 11) to range(1, 12)

    def on_cash_flow_calendar_fim_selected(self, obj):
        self.year_fim, self.month_fim, self.day_fim = obj.get_date()
        self.month_fim += 1 #Change range(0, 11) to range(1, 12)

    def on_cash_flow_report_generated_clicked(self, obj):
        opendebtsotherValor = 0
        cashflowValor = 0
        self.cashflowStore.clear()

        if datetime.date(int(self.year_ini), int(self.month_ini), int(self.day_ini)) > datetime.date(int(self.year_fim), int(self.month_fim), int(self.day_fim)):
            ok_only(_("<b>É necesssário que a Data fim seja maior ou igual a Data início.</b>\n\n"), Parent=None, ICON=gtk.MESSAGE_ERROR)
            return
        for line in self.manager.cash_flow_manager.get_debts_by_date(self.year_ini,self.month_ini,self.day_ini,self.year_fim,self.month_fim,self.day_fim):
            if line[1] == 4:
                cashflowValor -= line[0]
            else:
                cashflowValor += line[0]

        for line in self.manager.open_debts_other_manager.get_debts_by_date(self.year_ini,self.month_ini,self.day_ini,self.year_fim,self.month_fim,self.day_fim):
            opendebtsotherValor += line[3]

        for line in self.manager.open_debts_machine_manager.get_debts_by_date(self.year_ini,self.month_ini,self.day_ini,self.year_fim,self.month_fim,self.day_fim):
            opendebtsotherValor += line[0]

        self.cashflowStore.append(None, ["R$: %.2f" % cashflowValor, "R$: %.2f" % opendebtsotherValor])
    def on_cancel_button_clicked(self, obj):
        self.consulta_movimento.destroy()

class fatura_frequentador:

    def __init__(self, manager):

        self.manager = manager
        self.users_manager = self.manager.users_manager
        self.xml = get_gtk_builder('fatura_frequentador')
        self.fatura_frequentador = self.xml.get_object('window2')
        self.table = self.xml.get_object('table1')
        self.total = self.xml.get_object('table2')
        self.cols = ['Data','Serviço','Valor Total','Valor Pago','Valor a ser Pago']
        sequence = [str] * len(self.cols)
        self.usuario_list = self.users_manager.get_all()
        self.opendebtsStore = gtk.TreeStore(* sequence)
        self.opendebtsView = gtk.TreeView(self.opendebtsStore)
        self.nick = ''

        self.btn_print = self.xml.get_object("btn_print")
        self.btn_pesquisar = self.xml.get_object("button1")
        self.btn_print.set_sensitive(False)
        self.btn_pesquisar.set_sensitive(False)
    ...
    def run(self):
        ...
        self.total.attach(child=self.label,
                 left_attach=0,
                 right_attach=1,
                 top_attach=0,
                 bottom_attach=1,
        ...
        self.cancel_btn = gtk.Button(stock=gtk.STOCK_CANCEL)
        self.cancel_btn.connect("clicked", self.on_cancel_button_clicked)
        self.opendebtsView.cell = [None] * len(self.cols)
        self.opendebtsView.set_grid_lines(gtk.TREE_VIEW_GRID_LINES_BOTH)
        tvcolumn = [None] * len(self.cols)

        for colnum, col in enumerate(self.cols):
            self.opendebtsView.cell[colnum] = gtk.CellRendererText()
            tvcolumn[colnum] = gtk.TreeViewColumn(col, self.opendebtsView.cell[colnum])
            tvcolumn[colnum].add_attribute(self.opendebtsView.cell[colnum], 'text', colnum)
            self.opendebtsView.append_column(tvcolumn[colnum])

        scrollTree = self.xml.get_object("scrolledwindow")
        scrollTree.set_policy(gtk.POLICY_NEVER, gtk.POLICY_AUTOMATIC)
        scrollTree.add(self.opendebtsView)

        self.usuariosComboBox.show()
        self.opendebtsView.show()
        self.fatura_frequentador.show()
    ...
    def usuariosComboBoxValueChange(self, combobox):
        index = combobox.get_active()
        self.btn_pesquisar.set_sensitive(True)

        return

    def on_printdebtsuser_report_generated_clicked(self, obj):
         print_text = "Teste de Impressao"
         printer = PrintBase.on_print_preview(self.header)

    def on_opendebtsuser_report_generated_clicked(self, obj):
        date_ini = [int(self.year_ini),int(self.month_ini),int(self.day_ini)]
        date_fim = [int(self.year_fim),int(self.month_fim),int(self.day_fim)]
        restCashValor = 0
        self.btn_print.set_sensitive(False)
        self.opendebtsStore.clear()
        #out = self.users_manager.get_credit_and_id(self.nick)
        selected_index = self.usuariosComboBox.get_active()
        selected_user = self.usuario_list[selected_index]

        self.data = ''
        self.header = ''
        strtotal = "Total: " + str("R$: %.2f" % 0)

        if date(date_ini[0], date_ini[1], date_ini[2]) > date(date_fim[0], date_fim[1], date_fim[2]):
            ok_only(_("<b>Período de pesquisa inválido. Data inical não pode ser maior que a data final.</b>\n\n"), Parent=None, ICON=gtk.MESSAGE_ERROR)
            return True
        ...
        if selected_user:
            days = []
            for line in self.manager.open_debts_other_manager.get_debts_by_user_date(selected_user.id, date_ini, date_fim): #Itens de debitos outros
                date_time = datetime.date(int(line[0]), int(line[1]), int(line[2]))
                string_data = str(line[2]) + '/' + str(line[1]) + '/' + str(line[0])
                cashflowValor = 0
                if not date_time in days: #Testa se já usou o pagamentos do dia
                    days.append(date_time)
                    for cash in self.manager.cash_flow_manager.get_debts_by_utilizacao(int(line[0]), int(line[1]), int(line[2]), int(line[0]), int(line[1]), int(line[2])):
                        cashflowValor += (cash[0] + restCashValor)
                elif cashflowValor < 0: #Testa se há saldo a devolver
                    cashflowValor = restCashValor * -1

                self.btn_print.set_sensitive(True)
                restCashValor += float(line[5]) - cashflowValor
                self.opendebtsStore.append(None, [ ("%s-%s-%s" % (line[2], line[1], line[0])), 'Outros', "R$: %.2f" % line[5], "R$: %.2f" % cashflowValor, "R$: %.2f" % (line[5] - cashflowValor)])
                self.data = self.data + str(date_time) + '     Outros         ' + str("R$: %.2f" % line[5]) + ' -    ' + str("R$: %.2f" % cashflowValor) + ' -   ' + str("R$: %.2f" % (line[5] - cashflowValor)) + '\n'
                strtotal = "Total: " + str("R$: %.2f" % restCashValor)

            for line in self.manager.open_debts_machine_manager.get_debts_by_user_date(selected_user.id, date_ini, date_fim): #Itens de debitos maquinas
                date_time = datetime.date(int(line[0]), int(line[1]), int(line[2]))
                cashflowValor = 0
                if not date_time in days: #Testa se já usou o pagamentos do dia em outra situaçao
                    days.append(date_time)
                    for cash in self.manager.cash_flow_manager.get_debts_by_utilizacao(int(line[0]), int(line[1]), int(line[2]), int(line[0]), int(line[1]), int(line[2])): #recupera o pagamento do dia
                        cashflowValor += (cash[0] + restCashValor)
                elif cashflowValor < 0: #Testa se há saldo a devolver
                    cashflowValor = restCashValor * -1
                ...
                self.btn_print.set_sensitive(True)
                restCashValor += float(line[5]) - cashflowValor
                self.opendebtsStore.append(None, [ ("%s-%s-%s" % (line[2], line[1], line[0])), 'Internet', "R$: %.2f" % line[5], "R$: %.2f" % cashflowValor, "R$: %.2f" % (line[5] - cashflowValor)])
                self.data = self.data + str("%s-%s-%s" % (line[2], line[1], line[0])) + '     Internet         ' + str("R$: %.2f" % line[5]) + ' -    ' + str("R$: %.2f" % cashflowValor) + ' -   ' + str("R$: %.2f" % (line[5] - cashflowValor)) + '\n'
                strtotal = "Total: " + str("R$: %.2f" % restCashValor)

            self.label.set_markup("<b>" + strtotal + '</b>')

            self.total.show_all()
            self.header = 'Relatório de Débitos em Aberto\n\r Usuário: ' + self.nick + '     Periodo: ' + str("%s-%s-%s" % (date_ini[2], date_ini[1], date_ini[0])) + ' à ' + str("%s-%s-%s" % (date_fim[2], date_fim[1], date_fim[0])) + '\n\r'
            self.header += self.data
            self.header += '\n\r' + strtotal
        else:
          ok_only(_("<b>Usuário não é válido.</b>\n\n"), Parent=None, ICON=gtk.MESSAGE_ERROR)

    def on_cancel_button_clicked(self, obj):
        self.fatura_frequentador.destroy()

class relatorio_utilizacao_maquina:

    def __init__(self, manager):
        self.manager = manager
        self.xml = get_gtk_builder('relatorio_utilizacao_machine')
        self.bens_list = self.manager.machine_manager.get_all()
        self.relatorio = self.xml.get_object('relatorio_utilizacao_machine')
        self.table = self.xml.get_object('table1')
        self.total = self.xml.get_object('table2')
        self.cancel_bnt = self.xml.get_object('cancel_bnt')
        self.xml.connect_signals(self)
    ...
    def run(self):
        self.table.attach(self.data_inicio, 1, 2, 1, 2)
        self.table.attach(self.data_fim, 2, 3, 1, 2)
	...
	data = []
        for list_index, bem in enumerate(self.bens_list):
	    if not bem.id in data:
		data.append(bem.id)
            	self.bensListStore.append((int(list_index), "Máquina " + str(bem.id)))
    ...
    def on_pesquisar_clicked(self, obj):
        ...
        selected_index = self.bensComboBox.get_active()
        selected_bem = self.bens_list[selected_index]
        result = self.manager.history_manager.query_relatorio_utilizacao_machine(selected_bem.id, str_inicio, str_fim)
        try:
            self.xml.get_object('vbox1').get_children()[2].destroy()
        except Exception, error:
            print error
        ...
        restCashValor = 0
        strtotal = ''
        total_maq = datetime.timedelta()
        for line in result: #Itens de debitos outros
            date_time = datetime.date(int(line[2]), int(line[1]), int(line[0]))
            ...
            if segundo > 30:
                minuto = minuto + 1
            price_per_hour = self.manager.machine_category_manager.get_price_by_machine_id(line[5])
            if not price_per_hour:
                price_per_hour = get_default_client().get_float('price_per_hour')

            cashflowValor = calculate_credit(price_per_hour, hora, minuto)
            delta = datetime.timedelta()
            total_maq +=  datetime.timedelta(hours=hora, minutes=minuto, seconds=0)
            delta = datetime.timedelta(hours=hora, minutes=minuto, seconds=0) / line[4]
            strdelta = str(delta).split(".")
            liststore.append([ ("%s-%s-%s" % (line[0], line[1], line[2])), str(datetime.timedelta(hours=hora, minutes=minuto, seconds=0)), str(line[4]), strdelta[0]])
            strtotal = "Total Utilização: %s" % total_maq


        column1 = gtk.TreeViewColumn("Data Utilização")
        treeview.append_column(column1)
        column2 = gtk.TreeViewColumn("Tempo de utilização")
        treeview.append_column(column2)
        column3 = gtk.TreeViewColumn("Usuário(s)")
        treeview.append_column(column3)
        column4 = gtk.TreeViewColumn("Tempo Médio")
        treeview.append_column(column4)
...
class baixa_debts_utilizador:

    def __init__(self, manager):

        self.manager = manager
        self.users_manager = self.manager.users_manager
        self.open_debts_machine_manager = self.manager.open_debts_machine_manager
        self.open_debts_other_manager = self.manager.open_debts_other_manager
        self.cash_flow_manager = self.manager.cash_flow_manager
        self.xml = get_gtk_builder('buy_debts_utilizador')
        self.baixa_debts = self.xml.get_object('window2')
        self.table = self.xml.get_object('table1')
        self.total = self.xml.get_object('table2')
        self.cols = ['Data','Serviço','Hora Inicial','Hora Final','Valor do Serviço']
        #sequence = [str] * (len(self.cols)+1)#coluna adicional com checkbox
        self.opendebtsStore = gtk.TreeStore(str, str, str, str, str, bool)
        self.opendebtsView = gtk.TreeView(self.opendebtsStore)
        self.usuario_list = self.users_manager.get_all()
    ...
    def run(self):
        ...
        for list_index, usuario in enumerate(self.usuario_list):
            self.usuariosListStore.append((usuario.id, usuario.nick))
        ...
        renderer_in_out = gtk.CellRendererToggle()
        renderer_in_out.connect("toggled", self.on_toggled)
        column_in_out = gtk.TreeViewColumn("Débitos a Quitar", renderer_in_out, active=5)
        self.opendebtsView.append_column(column_in_out)
    ...
    def on_toggled(self, widget, path):
        current_value = self.opendebtsStore[path][4].split(':')
        debt_id = self.debts_ids[int(path)]
        values = debt_id.split(':')

        if self.opendebtsStore[path][5]:
            self.opendebtsStore[path][5] = False
            if values[1] == 'M':#Debitos Machine
                self.debts_machine.remove(values[0])
                self.strtotal -= float(current_value[1])
            else:
                self.debts_other.remove(values[0])
                self.strtotal -= float(current_value[1])
        else:
            self.opendebtsStore[path][5] = True
            if values[1] == 'O':#Debitos Outros
                self.debts_other.append(values[0])
                self.strtotal += float(current_value[1])
            else:
                self.debts_machine.append(values[0])
                self.strtotal += float(current_value[1])

        if len(self.debts_machine) > 0 or len(self.debts_other) > 0:
            self.btn_print.set_sensitive(True)
            self.label.set_markup("<b>" + str("Total: " + str("R$: %.2f" % self.strtotal)) + '</b>')
            self.total.show_all()
        else:
            self.btn_print.set_sensitive(False)
            self.total.hide()
    ...
    def on_printdebtsuser_report_generated_clicked(self, obj):
        for line in self.debts_machine:
            oitem = self.open_debts_machine_manager.get_all().filter_by(id=line).one()
            ...
            self.on_opendebtsuser_report_generated_clicked(self)

        for line in self.debts_other:
            oitem = self.open_debts_other_manager.get_all().filter_by(id=line).one()
            #Insert Entry in Cash Flow
            lctime = localtime()
            current_hour = "%0.2d:%0.2d:%0.2d" % lctime[3:6]
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if selected_user:
            for line in self.manager.open_debts_other_manager.get_debts_by_user_date(selected_user.id, date_ini, date_fim): #Itens de debitos outros
                ...
                cashflowValor = 0
                restCashValor += float(line[5]) - cashflowValor
                self.opendebtsStore.append(None, [("%s-%s-%s" % (line[2], line[1], line[0])), 'Outros', line[3], line[4], "R$: %.2f" % line[5], bool(False)])
                self.data = self.data + line[3] + '  ' + str("%s-%s-%s" % (line[2], line[1], line[0])) + '  Outros  ' + line[3] + ' - ' + line[4] + "     R$: %.2f" % line[5] + ' -  ' + str("R$: %.2f" % restCashValor) + '\n\n'
                self.debts_ids.append(str(line[6]) + ':' +'O')

            for line in self.manager.open_debts_machine_manager.get_debts_by_user_date(selected_user.id, date_ini, date_fim):
                date_time = datetime.date(int(line[0]), int(line[1]), int(line[2]))
                cashflowValor = 0
                restCashValor += float(line[5]) - cashflowValor
                self.opendebtsStore.append(None, [("%s-%s-%s" % (line[2], line[1], line[0])), 'Internet', line[3], line[4], "R$: %.2f" % line[5], bool(False)])
                self.data = self.data + line[3] + '  ' + str("%s-%s-%s" % (line[2], line[1], line[0])) + '  Internet  ' + line[3] + ' - ' + line[4] + "     R$: %.2f" % line[5] + ' -  ' + str("R$: %.2f" % restCashValor) + '\n\n'
                self.debts_ids.append(str(line[6]) + ':' +'M')

        else:
            ok_only(_("<b>Usuário não é válido.</b>\n\n"), Parent=None, ICON=gtk.MESSAGE_ERROR)

    def on_cancel_button_clicked(self, obj):
        self.baixa_debts.destroy()
###>BSI
...
class adduser(user_edit):
    ...
    def __init__(self, manager, price_per_hour, Parent=None):
        ...
        self.perfilComboBox.hide()
        self.xml.get_object('perfil_label').hide()

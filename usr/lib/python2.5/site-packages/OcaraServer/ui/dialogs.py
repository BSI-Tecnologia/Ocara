...
from OcaraServer.db.models import Action, ActionCategory
...
class user_edit:
    ...
    def __init__(self, manager, Parent=None):
        ...
        self.xml.get_object("perfil_label").set_mnemonic_widget(
            self.perfilComboBox)
...
class config_perfil:
    ...
    def actions_and_categories_treeStore_setup(self):
        ...
        for category in self.all_action_categories:
            ...
            for action in filter(lambda action: action.action_category_id == category.id, self.all_actions):
                ...
                lista_actions.append(action in self.all_perfil_actions)
    ...
    def run(self):
        ...
        books = self.action_and_categories_list #[["Tolstoy, Leo", ["War and Peace", True], ["Anna Karenina", True]],
        ...
        column_in_out = gtk.TreeViewColumn("Permitir acesso", renderer_in_out, active=1)
...
class lista_perfil:
	...
	perfilStore	= gtk.TreeStore(* sequence)
	perfilView	= gtk.TreeView(perfilStore)
	perfilView.cell	= [None] * len(cols)
	tvcolumn	= [None] * len(cols)
	for colnum, col in enumerate(cols):
	    perfilView.cell[colnum] = gtk.CellRendererText()
	    tvcolumn[colnum] = gtk.TreeViewColumn(col, perfilView.cell[colnum])
	    tvcolumn[colnum].add_attribute(perfilView.cell[colnum], 'text', colnum)
	    perfilView.append_column(tvcolumn[colnum])

        scrollTree = self.xml.get_object("scrollTree")
        scrollTree.set_policy(gtk.POLICY_NEVER, gtk.POLICY_AUTOMATIC)
	scrollTree.add(perfilView)

	for line in self.perfilList:
            print line
	    mother = perfilStore.append( None, [line.id, line.perfil_description])

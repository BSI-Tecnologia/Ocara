    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if out:
            ...
            for line in self.manager.open_debts_other_manager.get_debts_by_user_date(out[1], date_ini, date_fim): #Itens de debitos outros
                ...
                self.opendebtsStore.append(None, [ date_time, 'Outros', 'R$:' + str(line[5]), 'R$:' + str(cashflowValor), 'R$:' + str(restCashValor)])
                self.data = self.data + str(date_time) + '     Outros         R$:' + str(line[5]) + ' -    R$:' + str(cashflowValor) + ' -   R$:' + str(restCashValor) + '\n\r'
            ...
            for line in self.manager.open_debts_machine_manager.get_debts_by_user_date(out[1], date_ini, date_fim): #Itens de debitos maquinas
                ...
                self.btn_print.set_sensitive(True)
                restCashValor += int(line[5]) - int(cashflowValor)
                self.opendebtsStore.append(None, [ date_time, 'Internet', 'R$:' + str(line[5]), 'R$:' + str(cashflowValor), 'R$:' + str(restCashValor)])
                self.data = self.data + str(date_time) + '     Internet         R$:' + str(line[5]) + ' -    R$:' + str(cashflowValor) + ' -   R$:' + str(restCashValor) + '\n\r'
            self.header = 'Usuário: ' + self.nick + '\n\r Periodo: ' + str(date_ini) + ' à ' + str(date_fim) + '\n\r'
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if out:
            for line in self.manager.open_debts_other_manager.get_debts_by_user_date(out[1], date_ini, date_fim): #Itens de debitos outros
                ...
                cashflowValor = 0
                restCashValor += int(line[5]) - int(cashflowValor)
                self.opendebtsStore.append(None, [date_time, 'Outros', line[3], line[4], 'R$:' + str(line[5]), bool(False)])
                self.data = self.data + str(date_time) + '     Outros     ' + line[3] + ' - ' + line[4] + '    R$:' + str(line[5]) + ' -  R$:' + str(restCashValor) + '\n\n'
            ...
            for line in self.manager.open_debts_machine_manager.get_debts_by_user_date(out[1], date_ini, date_fim):
                ...
                cashflowValor = 0
                restCashValor += int(line[5]) - int(cashflowValor)
                self.opendebtsStore.append(None, [date_time, 'Internet', line[3], line[4], 'R$:' + str(line[5]), bool(False)])
                self.data = self.data + line[3] + '  ' + str(date_time) + '  Internet  ' + line[3] + ' - ' + line[4] + '    R$:' + str(line[5]) + ' -  R$:' + str(restCashValor) + '\n\n'
                self.debts_ids.append(str(line[6]) + ':' +'M')

            self.header = 'Usuário: ' + self.nick + '\n\r Periodo: ' + str(datetime.date(int(date_ini[0]), int(date_ini[1]), int(date_ini[2]))) + ' à ' + str(datetime.date(int(date_fim[0]), int(date_fim[1]), int(date_fim[2]))) + '\n\r'
            self.header += self.data
        else:
            ok_only(_("<b>Usuário não é válido.</b>\n\n"), Parent=None, ICON=gtk.MESSAGE_ERROR)

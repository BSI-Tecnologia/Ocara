...
class fatura_frequentador:
	...
	self.lbl_dt_ini = self.xml.get_object("lbl_dt_ini")
	self.lbl_dt_fim = self.xml.get_object("lbl_dt_fim")
        self.cancel_btn = self.xml.get_object('cancel_btn')
	...
	if out:
	    days = []
            for line in self.manager.open_debts_other_manager.get_debts_by_user(out[1]): #Itens de debitos outros
		...
		self.data = self.data + str(date_time) + '     Outros     ' + str(tempo_utilizado) + '    R$:' + str(line['value']) + ' -    R$:' + str(cashflowValor) + ' -   R$:' + str(restCashValor) + '\n\r'
            ...
            for line in self.manager.open_debts_machine_manager.get_debts_by_user(out[1]): #Itens de debitos maquinas
	    	date_time = datetime.date(int(line['year']), int(line['month']), int(line['day']))
	    	cashflowValor = 0
...
class baixa_debts_utilizador:
    ...
    def on_opendebtsuser_report_generated_clicked(self, obj):
        ...
        if nick == "":
            ...
            return
        if out:
            for line in self.manager.open_debts_machine_manager.get_debts_by_user(out[1]):
                date_time = datetime.date(int(line['year']), int(line['month']), int(line['day']))
                cashflowValor = 0
                restCashValor += int(line['value']) - int(cashflowValor)

...
class fatura_frequentador:
	...
	date_fim = datetime.datetime.now()
	restCashValor = 0
	...
	if out:
	    days = []
            for line in self.manager.open_debts_other_manager.get_debts_by_user(out[1]):
	    	date_time = datetime.date(int(line['year']), int(line['month']), int(line['day']))
	    	cashflowValor = 0
	    	if not date_time in days:
	    	...
	    	elif cashflowValor < 0:
	    	    cashflowValor = restCashValor * -1

	    	restCashValor += int(line['value']) - int(cashflowValor)
	    	self.opendebtsStore.append(None, [ date_time, line['id_description'], line['start_time'], line['value'], cashflowValor, restCashValor])
	else:

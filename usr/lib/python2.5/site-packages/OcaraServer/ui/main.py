...
class Manager:
    ...
    def __init__(self, daemon):
        ...
        self.opendebts_machine_bck_manager = self.daemon.opendebts_machine_bck_manager
        ###BSI
	...
	self.setup_btnRelatorios()
        ...
        dg = dialogs.authentication_dialog(self, False)
        dg.run()
    ...
    def on_baixa_debts_clicked(self, obj):
        if not self.has_access(57):  # action.id -> 57 = utilização de máquinas
            self.no_access_msg()
            return True

        dlg = dialogs.baixa_debts_utilizador(manager=self)
        dlg.run()
    ...
    def on_trocar_usuario_clicked(self, obj):
        ...
        dg = dialogs.authentication_dialog(self, True)
        dg.run()

    def on_utilizacao_bens_clicked(self, obj):
        if not self.has_access(54):  # action.id -> 54 = utilização de bens
            self.no_access_msg()
            return True

        dlg = dialogs.relatorio_utilizacao_bens(self)
        dlg.run()

    def on_utilizacao_frequentador_clicked(self, obj):
        if not self.has_access(56):  # action.id -> 56 = utilização de frequentadores
            self.no_access_msg()
            return True

        dlg = dialogs.relatorio_utilizacao_frequentador(self)
        dlg.run()

    def on_utilizacao_machine_clicked(self, obj):
        if not self.has_access(55):  # action.id -> 55 = utilização de máquinas
            self.no_access_msg()
            return True

        dlg = dialogs.relatorio_utilizacao_maquina(manager=self)
        dlg.run()
    ####BSI
    ...
    def on_consultar_movimento_clicked(self, obj):
        if not self.has_access(52):  # action.id -> 52 = consultar movimento de caixa
            self.no_access_msg()
            return True
    ...
    def on_fatura_frequentador_clicked(self, obj):
        if not self.has_access(53):  # action.id -> 53 = fatura de serviços não pagos
            self.no_access_msg()
            return True

        dlg = dialogs.fatura_frequentador(manager=self)
        dlg.run()
    #####################################
    ### NOTEBOOK FUNCTIONS
    #####################################
    ...
    def add_new_machine_category_clicked(self, obj):
        ...
        if not self.has_access(50):  # action.id -> 50 = adicionar categoria de maquina
            self.no_access_msg()
            return True
    ...
    def add_new_user_category_clicked(self, obj):
        ...
        if not self.has_access(51):  # action.id -> 51 = adicionar categoria de usuario
            self.no_access_msg()
            return True
    ...
    def setup_btnRelatorios(self):
        menu = gtk.Menu()
        labels = ['Consultar Movimento de Caixa','Fatura de Serviços não pagos', 'Utilização por Bens', 'Utilização por Frequentador','Utilização por Máquina','Baixar Débitos Frequentador']
        callbacks = [self.on_consultar_movimento_clicked, self.on_fatura_frequentador_clicked, self.on_utilizacao_bens_clicked, self.on_utilizacao_frequentador_clicked, self.on_utilizacao_machine_clicked, self.on_baixa_debts_clicked]
        items = [gtk.MenuItem(label=l) for l in labels]

...
class Manager:
    ...
    def on_listbens_tgbnt_toggled(self, obj):
        if obj.get_active():
            if self.check_access_tgbnt_mnu(39): # action_id -> 39 = lista de bens
    ...
    def check_access_tgbnt_mnu(self, action_id):
        for perfil_action in self.user_credentials:
            if perfil_action.action_id == action_id:
                return True
        dialogs.ok_only("Voçê não tem acesso a essa funcionalidade.", Parent=self.mainwindow, ICON=gtk.MESSAGE_ERROR)
        return False

    def show_access_not_allowed(self, action_id):

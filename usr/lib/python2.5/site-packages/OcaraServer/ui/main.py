...
class Manager:
    ...
    def block_machine_clicked(self, obj):
        ...
        if not self.has_access(4):  # action.id -> 4 = desconectar usuario
            self.no_access_msg()
            return True
    ...
    def on_view_history_machine(self, obj):
        ...
        if not self.has_access(8):  # action.id -> 4 = ver historico computador
            self.no_access_msg()
            return True
    ...
    def on_cash_flow_tgl_clicked(self, obj):
        if obj.get_active():
            self.show_access_not_allowed_tgbnt_mnu(30) # action.id -> 28 = lista de movimento de caixa
